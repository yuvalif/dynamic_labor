...
if (sex == 0)   % H=0
    ...
    JOB_OFFER = 1;
    tmp1_h = ability_h + beta10_h*(HE*H_HSD)+ beta11_h*(HE*H_HSG)+ beta12_h*(HE*H_SC)+ beta13_h*(HE*H_CG)+beta14_h*(HE*H_PC)+...
    ...
    if PROB_W > w_draws(draw_f,t,school_group,1)
        JOB_OFFER = 1;
        tmp1_w = ability_w + beta11_w*(WE*HSG)+ beta12_w*(WE*SC)+ beta13_w*(WE*CG)+ beta14_w*(WE*PC) +...
