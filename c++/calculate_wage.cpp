...
double calculate_wage_h(const Parameters& p, const Husband& husband, double epsilon) {
    ...
    const double tmp2 = epsilon*p.sigma[0];
...
double calculate_wage_w(const Parameters& p, const Wife& wife, double w_draw, double epsilon) {
    ...
    const double PROB_TMP = p.row0_w*wife.emp_state + p.row11_w*wife.HSG + p.row12_w*wife.SC + p.row13_w*wife.CG + p.row14_w*wife.PC + p.row2_w*wife.WE;
