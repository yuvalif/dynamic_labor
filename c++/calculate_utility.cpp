...
Utility calculate_utility(const Parameters& p, const Emax& EMAX_W, const Emax& EMAX_H, unsigned N_KIDS, unsigned N_KIDS_H,
    ...
    if (M == 0  &&  CHOOSE_PARTNER == 0) {
        ...
        for (auto i = 0; i <= 10; ++i) {  //consumption share grid
            ...
            const auto UC_W1 = pow(women_cons_m1, p.alpha)/p.alpha + p.alpha1_w_m*(N_KIDS) + wife.Q + wife.similar_educ + p.alpha2_w*log(1.0+N_KIDS) + p.alpha3_w;
            const auto UC_W2 = pow(women_cons_m2, p.alpha)/p.alpha + p.alpha1_w_m*(N_KIDS) + wife.Q + wife.similar_educ;
            const auto UC_H1 = pow(men_cons_m1, p.alpha)/p.alpha   + p.alpha1_h_m*(N_KIDS) + wife.Q + wife.similar_educ;
            const auto UC_H2 = pow(men_cons_m2, p.alpha)/p.alpha   + p.alpha1_h_m*(N_KIDS) + wife.Q + wife.similar_educ;
            if (t == T_END) {
                ...
                result.U_W[1+i] = UC_W1+p.t1_w*HSG+p.t2_w*SC+p.t3_w*CG+p.t4_w*PC+p.t5_w*wife.WE        + p.t6_w*H_HSD+p.t7_w*H_HSG+p.t8_w*H_SC+p.t9_w*H_CG+
                    p.t10_w*H_PC+p.t11_w*(HE+1.0)+p.t12_w+p.t13_w*(N_KIDS)+p.t14_w*(wife.Q+wife.similar_educ)+p.t15_w*(N_KIDS)+p.t17_w;

                result.U_W[12+i] = UC_W2+p.t1_w*HSG+p.t2_w*SC+p.t3_w*CG+p.t4_w*PC+p.t5_w*(wife.WE+1.0) + p.t6_w*H_HSD+p.t7_w*H_HSG+p.t8_w*H_SC+p.t9_w*H_CG+
                    p.t10_w*H_PC+p.t11_w*(HE+1.0)+p.t12_w+p.t13_w*(N_KIDS)+p.t14_w*(wife.Q+wife.similar_educ)+p.t15_w*(N_KIDS)+p.t16_w+p.t17_w;

                result.U_H[1+i] = UC_H1+p.t1_h*HSG+p.t2_h*SC+p.t3_h*CG+p.t4_h*PC+p.t5_h*wife.WE        + p.t6_h*H_HSD+p.t7_h*H_HSG+p.t8_h*H_SC+p.t9_h*H_CG+
                    p.t10_h*H_PC+p.t11_h*(HE+1.0)+p.t12_h+p.t13_h*(N_KIDS)+p.t14_h*(wife.Q+wife.similar_educ)+p.t17_h;

                result.U_H[12+i] = UC_H2+p.t1_h*HSG+p.t2_h*SC+p.t3_h*CG+p.t4_h*PC+p.t5_h*(wife.WE+1.0) + p.t6_h*H_HSD+p.t7_h*H_HSG+p.t8_h*H_SC+p.t9_h*H_CG+
                    p.t10_h*H_PC+p.t11_h*(HE+1.0)+p.t12_h+p.t13_h*(N_KIDS)+p.t14_h*(wife.Q+wife.similar_educ)+p.t16_h+p.t17_h;
            } else {
                // t < T_END
                ...
                value_to_index(wife.WE, HE+1, N_KIDS, BP, CS, exp_wi, exp_hi, kidsi, BPi, CSi);
                result.U_W[1+i] = UC_W1+beta0*EMAX_W[t+1][exp_wi][exp_hi][kidsi][UNEMP][wife.ability_wi][ability_hi][MARRIED][HS_IDX][WS_IDX][wife.Q_INDEX][BPi];
                result.U_H[1+i] = UC_H1+beta0*EMAX_H[t+1][exp_wi][exp_hi][kidsi][UNEMP][wife.ability_wi][ability_hi][MARRIED][HS_IDX][WS_IDX][wife.Q_INDEX][BPi];
                // calculate EMAX indexes when women employed
                value_to_index(wife.WE+1, HE+1, N_KIDS, BP, CS, exp_wi, exp_hi, kidsi, BPi, CSi);
                result.U_W[12+i] = UC_W2+beta0*EMAX_W[t+1][exp_wi][exp_hi][kidsi][EMP] [wife.ability_wi][ability_hi][MARRIED][HS_IDX][WS_IDX][wife.Q_INDEX][BPi];
                result.U_H[12+i] = UC_H2+beta0*EMAX_H[t+1][exp_wi][exp_hi][kidsi][EMP] [wife.ability_wi][ability_hi][MARRIED][HS_IDX][WS_IDX][wife.Q_INDEX][BPi];
            }
        }
        ...
        if (t == T_END) {
            result.U_W_S[1] = UC_W_S[1]+p.t1_w*HSG+p.t2_w*SC+p.t3_w*CG+p.t4_w*PC+p.t5_w*wife.WE    +p.t13_w*N_KIDS;     
            result.U_W_S[2] = UC_W_S[2]+p.t1_w*HSG+p.t2_w*SC+p.t3_w*CG+p.t4_w*PC+p.t5_w*(wife.WE+1.0)+p.t13_w*N_KIDS+p.t16_w;       
            result.U_H_S    = UC_H_S+p.t6_h*H_HSD+p.t7_h*H_HSG+p.t8_h*H_SC+p.t9_h*H_CG+p.t10_h*H_PC+p.t11_h*(HE+1.0)+p.t13_h*N_KIDS_H; 
        } else {
            unsigned exp_wi; unsigned exp_hi; unsigned kidsi; unsigned BPi;	unsigned CSi;
            value_to_index(wife.WE, 0, N_KIDS, 0, 0, exp_wi, exp_hi, kidsi, BPi, CSi);
            result.U_W_S[1] = UC_W_S[1]+beta0*EMAX_W[t+1][exp_wi][1][kidsi][UNEMP][wife.ability_wi][1][UNMARRIED][1][WS_IDX][1][1];

            value_to_index(wife.WE+1, 0, N_KIDS, 0, 0, exp_wi, exp_hi, kidsi, BPi, CSi);
            result.U_W_S[2] = UC_W_S[2]+beta0*EMAX_W[t+1][exp_wi][1][kidsi][EMP][wife.ability_wi][1][UNMARRIED][1][WS_IDX][1][1];
