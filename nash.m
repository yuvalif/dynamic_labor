function BP = nash(const_values, U_W, U_H, U_W_S, U_H_S, BP)
...
[~, max_weighted_utility_index] = max(weighted_utility);      % max of weighted utility
...
if (max_nash_value> -999)%%there is at least one option with positive surplus for both
    if(max_nash_value_index==max_weighted_utility_index && max_nash_value> -999)  %
        ...
        for tmp = 1:22
            weighted_utility_option(:,tmp)=U_H(tmp).*const_values.bp_vector+U_W(tmp).*(1-const_values.bp_vector);  %weighted utility
        end
