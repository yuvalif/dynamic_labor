function [U_W, U_H, U_W_S, U_H_S] = calculate_utility(N_KIDS, N_KIDS_H, wage_h, wage_w, CHOOSE_PARTNER, CHOOSE_WORK_H, CHOOSE_WORK_W, M, ...
                                                        similar_educ, Q, Q_INDEX, HS, WS, t, ability_hi, ability_wi, HE, WE, BP, T_END)  
...
if (M == 0  &&  CHOOSE_PARTNER == 0)
    ...
    for i = 0:10  % consumption share grid
        ...
        elseif (t < T_END)   % the loop goes from 28 to 1, but for SC, CG and PC the loop is shorter
            ...
            U_W(1,1+i) =UC_W1 +beta0*EMAX_W(t+1, exp_wi, exp_hi, kidsi, UNEMP, ability_wi, ability_hi, MARRIED, HS ,WS-1, Q_INDEX, BPi, CSi);
            U_H(1,1+i) =UC_H1 +beta0*EMAX_H(t+1, exp_wi, exp_hi, kidsi, UNEMP, ability_wi, ability_hi, MARRIED, HS ,WS-1, Q_INDEX, BPi, CSi);
            [exp_wi, exp_hi, kidsi, BPi, CSi] = value_to_index(WE+1, HE+1, N_KIDS, BP, CS);%calculate EMAX indexes when women employed            
            U_W(1,12+i)=UC_W2 +beta0*EMAX_W(t+1, exp_wi, exp_hi, kidsi,   EMP, ability_wi, ability_hi, MARRIED, HS ,WS-1, Q_INDEX, BPi, CSi);
            U_H(1,12+i)=UC_H2 +beta0*EMAX_H(t+1, exp_wi, exp_hi, kidsi,   EMP, ability_wi, ability_hi, MARRIED, HS ,WS-1, Q_INDEX, BPi, CSi);
...
elseif ( t < T_END)
    ...
    U_W_S(1,1) = UC_W_S(1,1)+beta0*EMAX_W(t+1, exp_wi, 1,      kidsi, UNEMP, ability_wi, 1,          UNMARRIED, 1,  WS-1, 1, 1, 1);   
    [exp_wi, exp_hi, kidsi, BPi, CSi] = value_to_index(WE+1, 0, N_KIDS,   0, 0);
    U_W_S(1,2) = UC_W_S(1,2)+beta0*EMAX_W(t+1, exp_wi, 1,      kidsi, EMP,   ability_wi, 1,          UNMARRIED, 1,  WS-1, 1, 1, 1);
    [exp_wi, exp_hi, kidsi, BPi, CSi] = value_to_index(0, HE+1, N_KIDS_H, 0, 0);
    U_H_S      = UC_H_S     +beta0*EMAX_H(t+1, exp_wi, exp_hi, kidsi, UNEMP, 1,          ability_hi, UNMARRIED, HS, WS-1, 1, 1, 1);
end
