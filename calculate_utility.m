function [U_W,U_H,U_W_S,U_H_S]=calculate_utility(N_KIDS, N_KIDS_H, wage_h, wage_w, CHOOSE_HUSBAND, CHOOSE_WORK_H, CHOOSE_WORK_W, M, STATIC, COUNTER, similar_educ, Q,Q_INDEX,HS, t, ability_h, ability_hi,ability_w, ability_wi, HE,WE,BP)  
...
women_cons_s(2) = wage_w*(1+(N_KIDS)*0.4);%women private consumption when single and employed
...
for i = 0:10  % consumption share grid
    CS = i*0.1;
    total_cons(1,1+i)  = ((total_earning(1,1)*(1.7+(N_KIDS)*0.4)).^hp / (CS.^hp+(1-CS).^hp)  ).^(1/hp); % only men employed
    total_cons(1,12+i) = ((total_earning(1,2)*(1.7+(N_KIDS)*0.4)).^hp / (CS.^hp+(1-CS).^hp)  ).^(1/hp); % both employed
    ...
    UC_W(1,1+i) =((women_cons_m(1,1 +i)).^alpha)/alpha  + alpha1_w_m*(N_KIDS)+  +Q+similar_educ + alpha2_w*log(1+N_KIDS) + alpha3_w;
    UC_W(1,12+i)=((women_cons_m(1,12+i)).^alpha)/alpha  + alpha1_w_m*(N_KIDS)+  +Q+similar_educ ;
    UC_H(1,1+i) =((men_cons_m(1,1 +i)).^alpha)/alpha  + alpha1_h_m*(N_KIDS)+  +Q+similar_educ  ;
    UC_H(1,12+i)=((men_cons_m(1,12+i)).^alpha)/alpha  + alpha1_h_m*(N_KIDS)+  +Q+similar_educ ;
    if (t == T)
        ...
        U_W(1,1+i) = UC_W(1,1+i) +t1_w*HSG+t2_w*SC+t3_w*CG+t4_w*PC+t5_w*WE    +t6_w*H_HSD+t7_w*H_HSG+t8_w*H_SC+t9_w*H_CG+t10_w*H_PC+t11_w*(HE+1)+t12_w+t13_w*(N_KIDS)+t14_w*(Q+similar_educ)+t15_w*(N_KIDS)+t17_w;     
        U_W(1,12+i)= UC_W(1,12+i)+t1_w*HSG+t2_w*SC+t3_w*CG+t4_w*PC+t5_w*(WE+1)+t6_w*H_HSD+t7_w*H_HSG+t8_w*H_SC+t9_w*H_CG+t10_w*H_PC+t11_w*(HE+1)+t12_w+t13_w*(N_KIDS)+t14_w*(Q+similar_educ)+t15_w*(N_KIDS)+t16_w+t17_w;    
        U_H(1,1+i) = UC_H(1,1+i) +t1_h*HSG+t2_h*SC+t3_h*CG+t4_h*PC+t5_h*WE    +t6_h*H_HSD+t7_h*H_HSG+t8_h*H_SC+t9_h*H_CG+t10_h*H_PC+t11_h*(HE+1)+t12_h+t13_h*(N_KIDS)+t14_h*(Q+similar_educ)+t17_h;       
        U_H(1,12+i)= UC_H(1,12+i)+t1_h*HSG+t2_h*SC+t3_h*CG+t4_h*PC+t5_h*(WE+1)+t6_h*H_HSD+t7_h*H_HSG+t8_h*H_SC+t9_h*H_CG+t10_h*H_PC+t11_h*(HE+1)+t12_h+t13_h*(N_KIDS)+t14_h*(Q+similar_educ)+t16_h+t17_h;  
    else
        ...
        [exp_wi   ,exp_hi, kidsi  ,   BPi   , CSi  ] = value_to_index(WE, HE+1, N_KIDS, BP, CS);%calculate EMAX indexes when women unemployed
       ...
       % size(EMAX_W)
        ...
        [exp_wi   ,exp_hi, kidsi  ,   BPi   , CSi  ] = value_to_index(WE+1, HE+1, N_KIDS, BP, CS);%calculate EMAX indexes when women employed            
...
UC_W_S(1,2) = ((women_cons_s(1,2)).^alpha)/alpha  + alpha1_w_s*(N_KIDS);
...
if (t == T)
    ...
    [exp_wi   ,exp_hi, kidsi  ,   BPi   , CSi  ] = value_to_index(WE  ,   0 , N_KIDS, 0,0);
    U_W_S(1,1)=UC_W_S(1,1)+beta0*EMAX_W(t+1,exp_wi   ,1     , kidsi  ,  1  , ability_wi,    1         ,     2  , 1         , 1,  1   , 1   );   
    [exp_wi   ,exp_hi, kidsi  ,   BPi   , CSi  ] = value_to_index(WE+1, 0   , N_KIDS,  0,0);
    U_W_S(1,2)=UC_W_S(1,2)+beta0*EMAX_W(t+1,exp_wi   ,1     , kidsi  ,  2  , ability_wi,    1         ,     2  , 1         , 1,  1   , 1   );
    [exp_wi   ,exp_hi, kidsi  ,   BPi   , CSi  ] = value_to_index(0   , HE+1, N_KIDS_H, 0,0);
